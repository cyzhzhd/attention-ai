...
export function analyzeFocus(video) {
  tfjs.enableProdMode();
  ...
  video.addEventListener("play", async () => {
    ...
    setInterval(async function faceAnalysis() {
      ...
      output = JSON.stringify(rowData(bbox, landmark, angle), null, 2);
      // analyze();
      drawAll(canvas, ctx, bbox, conf, landmark);
...
function drawAll(canvas, ctx, bbox, conf, landmarkObj) {
...
function drawInfo(ctx) {
  ...
  ctx.fillText("score: " + lines[-1], 10, 20);
  ctx.font = "12px Arial";
